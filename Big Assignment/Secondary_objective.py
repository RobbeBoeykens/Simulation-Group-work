...
OUTPUT_EXCEL = "Big Assignment/Excel Files/secondary_objectives.xlsx"
...
def write_batch_vr_sheet(wb, sheet_name, rows, meta):
    ...
    widths = [7, 12, 12, 14, 14, 14, 14, 12, 12, 14, 14, 14, 14, 14]
    ...
    for i, (label, value, unit) in enumerate(param_rows, start=3):
        ...
        ws.merge_cells(start_row=i, start_column=8, end_row=i, end_column=14)

    # Summary
    sum_start = 3 + len(param_rows) + 1
    ws.merge_cells(start_row=sum_start, start_column=1, end_row=sum_start, end_column=14)
    ...
    ws.merge_cells(start_row=formula_row, start_column=1, end_row=formula_row, end_column=14)
...
def main():
    ...
    for urgent, strategy, rule in DESIGNS:
        ...
        el_scan_n = compute_batch_means(get_post_warmup_metric_series(sim_normal, "movingAvgElectiveScanWT", RUN_WEEKS), M, L)
        el_scan_a = compute_batch_means(get_post_warmup_metric_series(sim_anti,   "movingAvgElectiveScanWT", RUN_WEEKS), M, L)
        ...
        ElScanWT = [(a + b) / 2.0 for a, b in zip(el_scan_n, el_scan_a)]
