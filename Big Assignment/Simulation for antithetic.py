...
RUN_WEEKS: int = 483
R: int = 16
TOTAL_WEEKS: int = WARMUP_WEEKS + RUN_WEEKS


def sheet_name_for(config: list[int]) -> str:
    slots, strategy, rule = config
    return f"S{strategy}-{slots}-R{rule}"


def input_file_for(config: list[int]) -> str:
    slots, strategy, _ = config
    return f"Big Assignment/Inputs/input-S{strategy}-{slots}.txt"
class Simulation:
    """
    Simulation instance
    ...
    avgUrgentScanWt: float
    avgOT: float
    normalOVs: list[float]
    antitheticOVs: list[float]
    pairedOVs: list[float]
    normalWeeklyOVs: list[list[float]]
    antitheticWeeklyOVs: list[list[float]]
    pairedWeeklyOVs: list[list[float]]
    numberOfElectivePatientsPlanned: int
    numberOfUrgentPatientsPlanned: int

    def __init__(self, filename: str, W: int, R: int, rule: int, useAntithetic: bool = False) -> None:
        """
        ...
        self.antitheticOVs = []
        self.pairedOVs = []
        self.normalWeeklyOVs = []
        self.antitheticWeeklyOVs = []
        self.pairedWeeklyOVs = []
    ...
    def generatePatients(self) -> None:
        ...
        When useAntithetic is True, the complementary exponential variates are used for the
        elective and urgent interarrival times.
        """
        arrivalTimeNext = 0.0
        counter = 0
        for w in range(self.W):
            for d in range(self.D):
                ...
                if (d < self.D - 1):  # not on sunday
                    interarrival, interarrivalAnti = Exponential_distribution(self.lambdaElective, both=True)
                    firstInterarrival = interarrivalAnti if self.useAntithetic else interarrival
                    arrivalTimeNext = 8 + firstInterarrival * (17 - 8)
                    while (arrivalTimeNext < 17):
                        ...
                        counter += 1
                        interarrival, interarrivalAnti = Exponential_distribution(self.lambdaElective, both=True)
                        nextInterarrival = interarrivalAnti if self.useAntithetic else interarrival
                        arrivalTimeNext += nextInterarrival * (17 - 8)
                ...
                if ((d == 3) or (d == 5)):
                    ...
                    endTime = 12
                interarrival, interarrivalAnti = Exponential_distribution(lmbd, both=True)
                firstInterarrival = interarrivalAnti if self.useAntithetic else interarrival
                arrivalTimeNext = 8 + firstInterarrival * (endTime - 8)
                while (arrivalTimeNext < endTime):
                    ...
                    interarrival, interarrivalAnti = Exponential_distribution(lmbd, both=True)
                    nextInterarrival = interarrivalAnti if self.useAntithetic else interarrival
                    arrivalTimeNext += nextInterarrival * (endTime - 8)
    ...
    def runSimulations(self) -> None:
        """
        Function that runs all the simulations using antithetic variates.
        Each base seed is evaluated twice: once with the original exponential interarrival
        variates and once with their antithetic counterparts. The objective values of the
        regular, antithetic, and paired runs are stored per replication for later export.
        """
        ...
        for r in range(self.R):
            ...
            self.resetSystem()
            self.useAntithetic = False
            random.seed(base_seed)
            self.runOneSimulation()
            regularElAppWT = self.avgElectiveAppWT
            regularElScanWT = self.avgElectiveScanWT
            regularUrScanWT = self.avgUrgentScanWt
            regularOT = self.avgOT
            regularOV = regularElAppWT * self.weightEl + regularUrScanWT * self.weightUr
            regularWeeklyOV = [
                self.movingAvgElectiveAppWT[w] * self.weightEl + self.movingAvgUrgentScanWT[w] * self.weightUr
                for w in range(self.W)
            ]

            self.resetSystem()
            self.useAntithetic = True
            random.seed(base_seed)
            self.runOneSimulation()
            antiElAppWT = self.avgElectiveAppWT
            antiElScanWT = self.avgElectiveScanWT
            antiUrScanWT = self.avgUrgentScanWt
            antiOT = self.avgOT
            antiOV = antiElAppWT * self.weightEl + antiUrScanWT * self.weightUr
            antiWeeklyOV = [
                self.movingAvgElectiveAppWT[w] * self.weightEl + self.movingAvgUrgentScanWT[w] * self.weightUr
                for w in range(self.W)
            ]

            pairElAppWT = (regularElAppWT + antiElAppWT) / 2
            pairElScanWT = (regularElScanWT + antiElScanWT) / 2
            pairUrScanWT = (regularUrScanWT + antiUrScanWT) / 2
            pairOT = (regularOT + antiOT) / 2
            pairOV = pairElAppWT * self.weightEl + pairUrScanWT * self.weightUr

            pairedWeeklyOV = [
                (regularWeeklyOV[w] + antiWeeklyOV[w]) / 2
                for w in range(self.W)
            ]

            self.normalOVs.append(regularOV)
            self.antitheticOVs.append(antiOV)
            self.pairedOVs.append(pairOV)
            self.normalWeeklyOVs.append(regularWeeklyOV)
            self.antitheticWeeklyOVs.append(antiWeeklyOV)
            self.pairedWeeklyOVs.append(pairedWeeklyOV)

            electiveAppWT += pairElAppWT
            electiveScanWT += pairElScanWT
            urgentScanWT += pairUrScanWT
            OT += pairOT
            OV += pairOV

            print(f"{r} \t {pairElAppWT:.2f} \t\t {pairElScanWT:.5f} \t {pairUrScanWT:.2f} \t\t {pairOT:.2f} \t {pairOV:.2f}")
        ...
        OT /= self.R
        OV /= self.R
        print("Stored normal OV values:", self.normalOVs)
        print("Stored antithetic OV values:", self.antitheticOVs)
        print("Stored paired OV values:", self.pairedOVs)
        print("--------------------------------------------------------------------------------")
...
def average_post_warmup_trajectory(replication: list[float]) -> float:
    post_warmup_values = replication[WARMUP_WEEKS:WARMUP_WEEKS + RUN_WEEKS]
    return sum(post_warmup_values) / len(post_warmup_values)

def export_all_configurations_to_excel(filename: str, W: int, R: int) -> None:
    """
    Run all configurations and export post-warmup trajectory averages to one Excel workbook.
    Each sheet corresponds to one configuration.

    Column 1: average normal OV over the post-warmup trajectory per replication
    Column 2: average antithetic OV over the post-warmup trajectory per replication
    Column 3: average paired OV over the post-warmup trajectory per replication

    Each sheet therefore contains one row per replication, not one row per run week.
    """
    ...
    for config in CONFIGURATIONS:
        input_file = input_file_for(config)
        sim = Simulation(input_file, W, R, config[2])
        sim.runSimulations()

        ws = wb.create_sheet(title=sheet_name_for(config))
        ws.cell(row=1, column=1, value="Normal OV")
        ws.cell(row=1, column=2, value="Antithetic OV")
        ws.cell(row=1, column=3, value="Paired OV")

        for replication_index in range(sim.R):
            avg_normal = average_post_warmup_trajectory(sim.normalWeeklyOVs[replication_index])
            avg_anti = average_post_warmup_trajectory(sim.antitheticWeeklyOVs[replication_index])
            avg_paired = average_post_warmup_trajectory(sim.pairedWeeklyOVs[replication_index])

            ws.cell(row=replication_index + 2, column=1, value=avg_normal)
            ws.cell(row=replication_index + 2, column=2, value=avg_anti)
            ws.cell(row=replication_index + 2, column=3, value=avg_paired)
...
if __name__ == "__main__":
    export_all_configurations_to_excel("Big Assignment/antithetic_OV_results.xlsx", TOTAL_WEEKS, R)
