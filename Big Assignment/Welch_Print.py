...
class Simulation:
    ...
    def runSimulations(self) -> None:
        ...
        for r in range(self.R):
            ...
            OT += self.avgOT
            replicationOV = self.avgElectiveAppWT * self.weightEl + self.avgUrgentScanWt * self.weightUr
            OV += replicationOV
            weeklyOV = []
            for week in range(self.W):
                weeklyOV.append(self.movingAvgElectiveAppWT[week] * self.weightEl + self.movingAvgUrgentScanWT[week] * self.weightUr)
            weeklyOVPerReplication.append(weeklyOV)
            print(f"{r} \t {self.avgElectiveAppWT:.2f} \t\t {self.avgElectiveScanWT:.5f} \t {self.avgUrgentScanWt:.2f} \t\t {self.avgOT:.2f} \t {replicationOV:.2f}")
